...
# [설정] 핀 번호 및 상태 파일
PIN = 14
STATE_FILE = os.path.expanduser("~/last_state_ac.txt")
LOCK_FILE = "/tmp/ac_relay_control.lock"
...
def show_header(current_state):
    os.system("clear")
    print("=" * 55)
    print(" [ AC 220V POWER CONTROL SYSTEM ] ".center(55))
    print("=" * 55)
    print("  ▶ 제어 모드    :  AC RELAY (Active Low)")
    print(f"  ▶ 연결 핀(BCM) :  GPIO {PIN} (물리 8번)")
    print(f"  ▶ 현재 상태    :  [\033[1;32m {current_state} \033[0m]")
    print("-" * 55)
    print("  사용 가능 명령어: [on] [off] [timer] [click] [exit]")
    print("-" * 55)


def level_for_action(action):
    return GPIO.LOW if action == "on" else GPIO.HIGH


def control(action, state_ref):
    value = level_for_action(action)
...
def click_test(state_ref, pulse_seconds=0.15):
    """
    릴레이 접점 동작(딸깍 소리) 확인용 짧은 펄스.
    현재 상태를 저장/복원하여 동작 영향 최소화.
    """
    current = state_ref["current"].lower()
    opposite = "off" if current == "on" else "on"

    print(f"\n  [테스트] {opposite.upper()} -> {current.upper()} 짧은 전환으로 릴레이 소리를 확인합니다.")
    GPIO.output(PIN, level_for_action(opposite))
    time.sleep(pulse_seconds)
    GPIO.output(PIN, level_for_action(current))
    print("  [테스트 완료] 릴레이 소리가 나지 않으면 배선/전원/모듈 상태를 확인하세요.")
...
def setup_gpio(initial_state):
    ...
    # 부팅/재실행 후 상태가 불안정해지는 경우를 줄이기 위해
    # 먼저 안전한 OFF(HIGH)로 고정한 뒤 목표 상태로 맞춤.
    GPIO.setup(PIN, GPIO.OUT, initial=GPIO.HIGH)
    time.sleep(0.05)

    target_level = GPIO.LOW if initial_state == "ON" else GPIO.HIGH
...
def main():
    ...
    try:
        while True:
            ...
            print("  ! 알 수 없는 명령입니다. [on/off/timer/click/exit] 중에서 입력하세요.")
