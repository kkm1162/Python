    ...
    for j, tag in enumerate(ordered_tags):
        ...
        for i in range(row_n):
            ...
            if tag == "compression-method":
                if iq16_row:
                    out, _ = remove_nth_tag(out, "compression-method", base + i)
                    out, _ = remove_nth_tag(out, "compression-type", base + i)
                    out, _ = remove_nth_tag(out, "exponent", base + i)
                    continue
...
def apply_physical_cc_rows(pdsch: str, pusch: str, prach: str, cc_rows: list[dict[str, Any]]) -> tuple[str, str, str, list[str]]:
    ...
    for i in range(min(16, len(cc_rows))):
        ...
        if prb is not None and str(prb).strip() != "":
            ...
            prach, ok = _replace_nth_tag(prach, "number-of-prb", pv, i)
            if not ok:
                warns.append(f"PRACH: could not replace number-of-prb index {i}")
...
def apply_global_baselines(xml: str, baselines: dict[str, str], live: dict[str, str]) -> str:
    ...
    # interface/name only (avoid touching every <name> in payload)
    cu_if = pick("cu_if_name")
    if cu_if:
        out = re.sub(
            r"(<interface>\s*<name>)([^<]*)(</name>)",
            lambda m: f"{m.group(1)}{cu_if}{m.group(3)}",
            out,
            count=1,
            flags=re.DOTALL,
        )
...
def load_workbook_payloads(
    ...
    try:
        ...
        # These workbook sheets are line-oriented XML; derive a FULL table view from RPC.
        for sheet in ("PDSCH", "PUSCH", "PRACH"):
            if sheet in wb.sheetnames:
                # Temporarily read rpc text for table building (below rpc[] fills too).
