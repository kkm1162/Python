#!/usr/bin/env bash
# O-RAN M-Plane 3.1.8.5 — SWM negative (FM data access check)
set -u
set -o pipefail
...
###############################################################################
# STEP 3: swmuser tries to get FM data — expect empty response
###############################################################################
GET_FM_RPC="${NETCONF_TMP}/get/get_fm.xml"
GET_FM_OUT="${NETCONF_TMP}/get/get_fm_out.xml"
cat > "$GET_FM_RPC" <<'EORPC'
...
rm -f "$GET_FM_OUT"
send_cmd "user-rpc --content $GET_FM_RPC --out $GET_FM_OUT"

RESULT3="NOK"
for _w in $(seq 1 50); do
	if [[ -f "$GET_FM_OUT" ]]; then
		if grep -aq "</data>" "$GET_FM_OUT" 2>/dev/null \
			|| grep -aq '<data xmlns="urn:ietf:params:xml:ns:netconf:base:1.0"/>' "$GET_FM_OUT" 2>/dev/null; then
			RESULT3="OK"
			break
		fi
	fi
...
if [[ "$RESULT3" == "OK" ]]; then
	if (( ! $(xmlstarlet sel -t -v "count(/*[local-name()='data']/*[local-name()])" -n "$GET_FM_OUT" 2>/dev/null) )); then
		RESULT3="OK"
	else
		RESULT3="NOK"
	fi
...
if [[ "$RESULT3" != "OK" ]]; then
	test_fail "swm-group FM data access check"
	exit 1
fi

echo "[PASS]"
echo "[INFO] 3.1.8.5 SWM negative completed. Detailed log: $LOG"
