...
def resolve_teraterm_path(configured=None):
    """설정 경로가 없거나 파일이 없으면 일반적인 TeraTerm 설치 위치를 탐색"""
    if configured and os.path.isfile(configured):
        return configured
    for path in TERATERM_CANDIDATES:
        if os.path.isfile(path):
            return path
    return configured or TERATERM_CANDIDATES[0]

def build_ssh_connect_string(ip, port, user, password):
    """TeraTerm 5 SSH connect 매크로 인자 생성"""
    parts = [f"{ip.strip()}:{port or '22'}", "/ssh", "/2"]
    if user or password:
        parts.append("/auth=password")
    if user:
        parts.append(f"/user={user.strip()}")
    if password:
        parts.append(f"/passwd={password.replace(chr(39), chr(39)+chr(39))}")
    return " ".join(parts)
...
class SecureData:
    ...
    def encrypt_password(password):
        if not password:
            return ""
        plain = SecureData.resolve_password(password)
        if not plain or SecureData.is_encrypted_password(plain):
    ...
    @staticmethod
    def resolve_password(value, max_layers=5):
        """평문 비밀번호를 반환. DPAPI 다중 래핑/레거시 Base64를 순서대로 해제"""
        if not value:
            return ""
        current = str(value)
        for _ in range(max_layers):
            if current.startswith(SecureData.DPAPI_PREFIX):
                decoded = SecureData.decrypt_password(current)
                if decoded == current:
                    break
                current = decoded
                continue
            try:
                plain = base64.b64decode(current.encode("ascii")).decode("utf-8")
                if plain and plain != current:
                    current = plain
                    continue
            except Exception:
                pass
    ...
    @staticmethod
    def is_encrypted_password(value):
        return bool(value) and str(value).startswith(SecureData.DPAPI_PREFIX)
...
class SessionRow:
    ...
    def update_ui_elements(self):
        ...
        if not self.outer_frame or not self.outer_frame.winfo_exists():
            return
        t = self.data.get('type', 'S')
        color = {"S": "#1976d2", "R": "#9c27b0", "M": "#2e7d32", "P": "#fbc02d", "C": "#e64a19"}.get(t, "#455a64")

        if self._color_bar and self._color_bar.winfo_exists():
            self._color_bar.config(bg=color)
        if self.lbl_name and self.lbl_name.winfo_exists():
        ...
        if t == "S":
            ...
            info = f"COM{info}"
        if self._lbl_info and self._lbl_info.winfo_exists():
            self._lbl_info.config(text=info)

        conn_text = CONNECT_TEXT_MAP.get(t, "CONNECT")
        if self._btn_conn and self._btn_conn.winfo_exists():
    ...
    def open_config(self):
        ...
        for l, k in flds:
            ...
            val = SecureData.resolve_password(self.data.get(k, '')) if "pw" in k else self.data.get(k, '')
        ...
        def save_config():
            ...
            self.app.invalidate_folder_counts()
            self.mark_ui_dirty()
    ...
    def run_connection(self, use_steps=False):
        """연결 실행"""
        d = self.data
        tt_path = resolve_teraterm_path(self.app.global_settings.get("teraterm_path"))
        ...
        user = d.get('user', '')
        pw = SecureData.resolve_password(d.get('pw', ''))
        ru_cmd = d.get('ru_cmd', '')
        ru_pw = SecureData.resolve_password(d.get('ru_pw', ''))
        ...
        try:
            ...
            if d['type'] in ["S", "C"]:
                ...
                with open(macro_path, 'w', encoding='cp949', errors='replace') as f:
                    ...
                    if d['type'] == "S":
                        ip_clean = d['ip'].strip()
                        if SecureData.is_encrypted_password(pw):
                            self.update_status("Error", "#f44336")
                            self._show_message(
                                "error", "Error",
                                "저장된 비밀번호를 복호화할 수 없습니다.\n"
                                "세션 설정(⚙)에서 비밀번호를 다시 입력한 뒤 저장하세요."
                            )
                            return

                        conn_str = build_ssh_connect_string(ip_clean, d.get('port', '22'), user, pw)
                ...
                if not os.path.isfile(tt_path):
                    ...
                    self._show_message(
                        "error", "Error",
                        f"TeraTerm을 찾을 수 없습니다.\n\n경로: {tt_path}\n\n"
                        "⚙ SET → TeraTerm 경로를 확인하세요.\n"
                        "예: C:\\Program Files\\teraterm5\\ttermpro.exe"
                    )
                    return
                ...
                try:
                    ...
                    self._show_message(
                        "error", "Error",
                        f"TeraTerm을 찾을 수 없습니다.\n\n경로: {tt_path}\n\n"
                        "⚙ SET → TeraTerm 경로에서 ttermpro.exe 위치를 확인하세요."
                    )
                except OSError as e:
                    self.update_status("Error", "#f44336")
                    self._show_message("error", "Error", f"TeraTerm 실행 실패:\n{e}")
    ...
    def on_drag_motion(self, e):
        ...
        if self.app.dragging_item and gw > 0:
            ...
            if self.get_col(self.app.current_folder) != tc: 
                ...
                self.app.schedule_drag_refresh()
...
class MainApp:
    ...
    def __init__(self, root):
        ...
        self._folder_counts_cache = None
        self._folder_counts_valid = False
        self._prev_selected_sessions = set()
        self._drag_refresh_after_id = None
        self._scroll_update_after_id = None
        self._save_after_id = None
        self._save_delay_ms = 250
        self._search_after_id = None
        self._search_delay_ms = 200
    ...
    def load_all(self):
            ...
            try:
                with open(DATA_FILE, 'r', encoding='utf-8') as f:
                    ...
                    for s in d.get("sessions", []):
                        if 'pw' in s:
                            s['pw'] = SecureData.resolve_password(s['pw'])
                        if 'ru_pw' in s:
                            s['ru_pw'] = SecureData.resolve_password(s['ru_pw'])
        ...
        resolved_tt = resolve_teraterm_path(self.global_settings.get("teraterm_path"))
        if resolved_tt and self.global_settings.get("teraterm_path") != resolved_tt:
            self.global_settings["teraterm_path"] = resolved_tt
        elif not self.global_settings.get("teraterm_path"):
            self.global_settings["teraterm_path"] = resolved_tt
    ...
    def invalidate_folder_counts(self):
        self._folder_counts_valid = False

    def get_folder_counts(self):
        if self._folder_counts_valid and self._folder_counts_cache is not None:
            return self._folder_counts_cache
        counts = {}
        for s in self.sessions:
            folder_name = s.data.get('folder')
            if folder_name:
                counts[folder_name] = counts.get(folder_name, 0) + 1
        self._folder_counts_cache = counts
        self._folder_counts_valid = True
        return counts

    def schedule_drag_refresh(self):
        if self._drag_refresh_after_id:
            try:
                self.root.after_cancel(self._drag_refresh_after_id)
            except Exception:
                pass
        self._drag_refresh_after_id = self.root.after(DRAG_REFRESH_DEBOUNCE_MS, self._run_drag_refresh)

    def _run_drag_refresh(self):
        self._drag_refresh_after_id = None
        self.refresh_grid_light()

    def _schedule_scroll_update(self):
        if self._scroll_update_after_id:
            return
        self._scroll_update_after_id = self.root.after_idle(self._update_scroll_region)
    ...
    def init_ui(self):
        ...
        self.canvas.bind("<Configure>", lambda e: (self.canvas.itemconfig(cw, width=e.width), self._schedule_scroll_update()))
    ...
    def refresh_grid(self):
        ...
        for folder_name in self.folder_order:
            if folder_name not in self.folders:
                continue
            count = folder_counts.get(folder_name, 0)
        ...
        for session in filtered_sessions:
            if session._ui_dirty:
                session.update_ui_elements()
            ...
            session.update_selection_visual()
            rc[cl] += 1
    ...
    def refresh_grid_light(self):
        """드래그 중 가벼운 갱신 (보이는 세션만 재배치)"""
        rc = [1] * self.col_count
        for s in self.get_filtered_sessions():
            cl = min(s.get_col(self.current_folder), self.col_count - 1)
            if s.outer_frame and s.outer_frame.winfo_exists():
    ...
    def apply_selection_visuals(self):
        ...
        to_update = set(self.selected_sessions) | self._prev_selected_sessions | set(self.get_filtered_sessions())
        self._prev_selected_sessions = set(self.selected_sessions)

        for s in to_update:
            try:
                new_selected = s in self.selected_sessions
                if s.is_selected == new_selected and not (s.outer_frame and s.outer_frame.winfo_exists()):
                    continue
                s.is_selected = new_selected
                if s.outer_frame and s.outer_frame.winfo_exists():
