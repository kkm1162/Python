...
# -*- coding: utf-8 -*-
"""
O-RAN O-RU DDoS Validation System v5.0 (God Mode Auto-Injection)
- VS Code 찌꺼기 텍스트 제거 및 완벽한 실행 보장
- 리눅스 서버 my_attack.py 갱신 실패 문제 원천 차단 (GUI에서 스크립트 직접 주입)
- 어떠한 코드 생략이나 간소화 없이 원본 구조 100% 유지
"""
...
class ORanValidationGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("O-RAN O-RU DDoS Validation System v5.0")
    ...
    def _build_ru_attack_tab(self):
        ...
        self.chk_spoofing = ttk.Checkbutton(atk_frame, text="Enable Source Spoofing (Random IP/MAC)", variable=self.var_spoofing)
        self.chk_spoofing.grid(row=4, column=0, columnspan=2, sticky="w", padx=10, pady=5)

        sim_frame = ttk.LabelFrame(atk_frame, text="Expected Throughput Simulation", padding="10")
        sim_frame.grid(row=5, column=0, columnspan=2, pady=10, sticky="ew")
        ...
        self.txt_desc = tk.Text(atk_frame, width=55, height=14, bg="#e8f6f3", font=("Malgun Gothic", 9), wrap=tk.WORD)
        self.txt_desc.grid(row=1, column=2, rowspan=5, padx=20, pady=5, sticky="nsew")
        self.txt_desc.config(state=tk.DISABLED)

        self.btn_build_pcap = tk.Button(atk_frame, text="1단계: 공격 패킷 생성 (PCAP Build)", 
                                        bg="#f39c12", fg="white", font=("Malgun Gothic", 11, "bold"), 
                                        command=self.build_pcap)
        self.btn_build_pcap.grid(row=6, column=0, columnspan=3, pady=15, ipadx=20, ipady=10)
    ...
    def _build_control_tab(self):
        ...
        self.combo_pcap_files = ttk.Combobox(pcap_frame, width=50, state="readonly")
        self.combo_pcap_files.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ...
        self.btn_play = tk.Button(ctrl_frame, text="▶ 2단계: 실제 트래픽 발사 (PLAY)", 
        ...
        self.btn_stop = tk.Button(ctrl_frame, text="⏹ 발사 긴급 중지 (STOP)", 
...
"""
    ...
    def _refresh_pcap_list(self):
        ...
        try:
            ...
            try:
                ...
                self.combo_pcap_files.config(values=sorted(files, reverse=True))
                if files: self.combo_pcap_files.current(0)
            except IOError:
                pw = self.ent_ssh_pw.get()
                stdin, stdout, stderr = self.ssh_client.exec_command(f"sudo -S mkdir -p {pcap_dir}", get_pty=True)
                stdin.write(pw + "\n")
                stdin.flush()
                self.combo_pcap_files.config(values=[])
            sftp.close()
    ...
    def _build_pcap_bg(self):
        try:
            ...
            # [핵심 개조] 리눅스 서버에 있는 낡은 my_attack.py를 완전히 무시하고,
            # GUI가 직접 가장 완벽한 파이썬 생성 엔진을 서버에 주입하여 실행합니다!
...
def build_packet(config):
    ...
    if 'PRACH' in atype:
        ...
        rtc_seq = b'\x00\x00\x00\x00'
        oran_hdr = b'\x00\x00\x00\x00\x01\x03'
        pad_len = max(0, ecpri_payload_len - 10)
...
"""
            ...
            self.ssh_client.exec_command(f"echo '{b64_builder}' | base64 -d > /tmp/oran_attack_builder.py")
            time.sleep(0.5)
    ...
    def play_traffic(self):
        ...
        selected_pcap = self.combo_pcap_files.get()
        if not selected_pcap: return
        ...
        stdin, stdout, stderr = self.ssh_client.exec_command(f"sudo -S sh -c \"echo '{target_rate} Gbps (목표치)' > /tmp/trex_target_rate\"", get_pty=True)
        stdin.write(pw + "\n")
        stdin.flush()

        pcap_target = f"{self.ent_pcap_path.get()}/{selected_pcap}"
...
try:
    ...
    c.clear_stats()
    stream = STLStream(packet=STLPktBuilder(pkt=raw_pkt), mode=STLTXCont(pps=100000))
    for p in ports_to_fire: c.add_streams([stream], ports=[p])
...
"""
        ...
        self.ssh_client.exec_command(f"echo '{b64_script}' | base64 -d > /tmp/run_fire.py")
        ...
        messagebox.showinfo("발사 확인", f"포트 {port_val}에서 트래픽 발사를 시작합니다!")
    ...
    def stop_traffic(self):
        ...
        stdin, stdout, stderr = self.ssh_client.exec_command(f"sudo -S sh -c \"echo '0 Gbps (중지됨)' > /tmp/trex_target_rate\"", get_pty=True)
        stdin.write(pw + "\n")
        stdin.flush()
...
"""
        ...
        self.ssh_client.exec_command(f"echo '{b64_stop}' | base64 -d > /tmp/stop_fire.py")
        stdin, stdout, stderr = self.ssh_client.exec_command("sudo -S python3 /tmp/stop_fire.py", get_pty=True)
        stdin.write(pw + "\n")
        stdin.flush()
