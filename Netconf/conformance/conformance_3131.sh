#!/usr/bin/env bash
# O-RAN M-Plane 3.1.3.1 — M-Plane supervision (positive): subscribe + 반복 supervision 알림 후 watchdog RPC
set -u
set -o pipefail
...
if [[ "$RESULT4" != "OK" ]]; then
	test_fail "supervision cycle"
	exit 1
fi

echo "[INFO] 3.1.3.1 positive path completed. Detailed log: $LOG"
trap - EXIT INT TERM
cleanup || true
if [[ -n "${NETOPEER_COPROC_PID:-}" ]]; then
