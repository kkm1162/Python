    ...
    if [[ -n "$out_path" ]]; then
        for _w in $(seq 1 600); do
            if [[ -s "$out_path" ]]; then
                {
                    flock -x 201
                    echo "[GUI] user-rpc reply xml begin ($out_path)" >> "$LOG" 2>&1
                    sed -n '1,1200p' "$out_path" >> "$LOG" 2>&1
                    echo "[GUI] user-rpc reply xml end ($out_path)" >> "$LOG" 2>&1
                } 201>"$CMD_LOCK_FILE"
                break
            fi
            sleep 0.1
        ...
        if [[ ! -s "$out_path" ]]; then
            {
                flock -x 201
                if [[ -e "$out_path" ]]; then
                    echo "[WARN] user-rpc reply out exists but empty: $out_path" >> "$LOG" 2>&1
                else
                    echo "[WARN] user-rpc reply out not produced within 60s: $out_path" >> "$LOG" 2>&1
                fi
            } 201>"$CMD_LOCK_FILE"
        fi
    fi
