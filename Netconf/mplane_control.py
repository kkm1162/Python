...
def _is_position_in_xml_comment(text: str, pos: int) -> bool:
    """True when ``pos`` falls inside an unclosed ``<!-- ... -->`` region."""
    if pos < 0:
        return False
    before = text[:pos]
...
def detect_commented_cc_row_indices_from_rpc(xml: str, kind: str) -> list[int]:
    """Return 1-based CC row indices whose endpoint blocks are wrapped in XML comments."""
    text = xml or ""
    if not text.strip():
        return []
    kind_u = kind.upper()
    ...
    pat = cc_pat if cc_pat.search(text) else block_pat
    off: list[int] = []
    if pat.search(text):
        for i, m in enumerate(pat.finditer(text), start=1):
            if _is_position_in_xml_comment(text, m.start()):
                off.append(i)
        return off
    _prefix, segments = _split_endpoint_segments(text, endpoint_tag)
    pos = 0
    for i, seg in enumerate(segments, start=1):
        st = text.find(seg, pos)
        if st >= 0 and _is_position_in_xml_comment(text, st):
            off.append(i)
        if st >= 0:
            pos = st + len(seg)
    return off
...
def _uncomment_wrapped_blocks(xml: str, inner_pat: re.Pattern[str]) -> str:
    out = xml or ""
    while True:
        new_out, n = inner_pat.subn(r"\1", out)
        out = new_out
        if n == 0:
            break
    return out
...
def uncomment_endpoint_rows(xml: str, kind: str) -> str:
    ...
    block_tag = "low-level-tx-endpoints" if kind_u == "PDSCH" else "low-level-rx-endpoints"
    link_tag = "low-level-tx-links" if kind_u == "PDSCH" else "low-level-rx-links"
    # ACORN per-CC chunk: endpoints … links (Apply comment-out uses this shape).
    cc_pat = re.compile(
        rf"<!--\s*(<\s*{re.escape(block_tag)}\b[\s\S]*?</\s*{re.escape(link_tag)}\s*>)\s*-->",
        re.DOTALL,
    )
    out = _uncomment_wrapped_blocks(text, cc_pat)
    out = _uncomment_block_comments(out, block_tag)
    ...
    out = _uncomment_wrapped_blocks(out, pat)
    ...
    out = _uncomment_wrapped_blocks(out, pat_leaf)
    return out
...
def _default_active_carrier_block(tag: str) -> str:
    return (
        f"        <{tag}>\n"
...
def _build_active_blocks_for_table(
    headers: list[str],
    rows: list[list[str]],
    existing: list[str],
    block_tag: str,
    ...
    """
  One ACTIVE carrier block per Control-Sheet row (index-aligned with PDSCH/PUSCH/PRACH).
  Grows or shrinks the worksheet template block list to match the table row count.
  """
    warns: list[str] = []
    if not rows:
        return list(existing), warns
    proto = existing[-1] if existing else _default_active_carrier_block(block_tag)
    if existing and len(existing) != len(rows):
        warns.append(
            f"ACTIVE: resized {block_tag} blocks from {len(existing)} to {len(rows)} "
            "(Control-Sheet row count)"
        )
    out: list[str] = []
    for i, row in enumerate(rows):
        seg = existing[i] if i < len(existing) else proto
        if _is_control_uplane_data_row(row):
            name = _array_carrier_name_from_row(headers, row, name_re)
            if name:
                seg = _upsert_first_tag(seg, "name", name)
        seg = _upsert_first_tag(seg, "active", "ACTIVE")
        out.append(seg)
    return out, warns


def sync_active_carrier_names_from_tables(
    active_xml: str,
    pdsch: tuple[list[str], list[list[str]]],
    pusch: tuple[list[str], list[list[str]]],
) -> tuple[str, list[str]]:
    """
    Align ACTIVE ``tx-array-carriers`` / ``rx-array-carriers`` with PDSCH/PUSCH detail rows:
    one carrier block per table row (same index as comment_out_endpoint_rows), sync names
    from the grid, and grow/shrink the template block list to match the row count.
  """
    warns: list[str] = []
    text = uncomment_active_rows((active_xml or "").strip())
    if not text:
    ...
    tx_tag = "tx-array-carriers"
    tx_blocks = _extract_closed_blocks(text, tx_tag)
    if not tx_blocks:
        tx_tag = "tx-array-carrier"
        tx_blocks = _extract_closed_blocks(text, tx_tag)

    rx_tag = "rx-array-carriers"
    rx_blocks = _extract_closed_blocks(text, rx_tag)
    if not rx_blocks:
        rx_tag = "rx-array-carrier"
        rx_blocks = _extract_closed_blocks(text, rx_tag)

    inner: list[str] = []
    if ph and pr:
        new_tx, tw = _build_active_blocks_for_table(ph, pr, tx_blocks, tx_tag, _TAC_CARRIER_RE)
        warns.extend(tw)
        inner.extend(new_tx)
    elif tx_blocks:
        inner.extend(tx_blocks)

    if uh and ur:
        new_rx, rw = _build_active_blocks_for_table(uh, ur, rx_blocks, rx_tag, _RAC_CARRIER_RE)
        warns.extend(rw)
        inner.extend(new_rx)
    elif rx_blocks:
        inner.extend(rx_blocks)
...
def _control_row_col_a_off(row: tuple) -> bool:
    if not row or len(row) < 1 or row[0] is None:
        return False
    return str(row[0]).strip().upper() in ("OFF", "0", "FALSE", "NO", "N", "DISABLE", "DISABLED")


def merge_uplane_detail_tables(
    ctrl: tuple[list[str], list[list[str]]] | None,
    derived: tuple[list[str], list[list[str]]] | None,
) -> tuple[list[str], list[list[str]]] | None:
    """
    Prefer Control-Sheet values but keep the full row count from uncommented RPC
    when the spreadsheet grid is shorter (e.g. OFF rows cleared or omitted).
    """
    if not derived or not derived[0] or not derived[1]:
        return ctrl
    if not ctrl or not ctrl[0] or not ctrl[1]:
        return derived
    ch, cr = ctrl
    dh, dr = derived
    if len(cr) >= len(dr):
    ...
    headers = dh if len(dh) >= len(ch) else ch
    hm = _col_map(headers)
    cm = _col_map(ch)
    rows: list[list[str]] = []
    for i in range(len(dr)):
        ...
        if len(base) < len(headers):
            base.extend([""] * (len(headers) - len(base)))
        if i < len(cr):
            for cn, ci in cm.items():
                hi = hm.get(cn)
                if hi is None or ci >= len(cr[i]):
                    continue
                val = cr[i][ci]
                if val is not None and str(val).strip():
                    base[hi] = str(val).strip()
        rows.append(base[: len(headers)])
    return headers, rows
...
def parse_control_uplane_detail_tables(
    ...
    for sec_key, kind in section_map.items():
        ...
        for j in range(header_i + 1, len(rows)):
            ...
            col_a_off = _control_row_col_a_off(row)
            if not any(cells):
                if col_a_off:
...
def load_workbook_payloads(
    ...
    try:
        ...
        for sheet in ("PDSCH", "PUSCH", "PRACH"):
            ...
            derived: tuple[list[str], list[list[str]]] | None = None
            rpc_raw = ""
            if sheet in wb.sheetnames:
                rpc_raw = read_sheet_rpc_text(wb[sheet]).strip()
                if rpc_raw:
                    try:
                        rpc_full = uncomment_endpoint_rows(rpc_raw, sheet)
                        derived = derive_full_table_from_rpc(rpc_full, sheet)
                    except Exception as exc:
                        warnings.append(f"[Workbook] Failed to derive table from {sheet}: {exc}")

            merged_tbl = merge_uplane_detail_tables(ctrl, derived)
            if merged_tbl and merged_tbl[0] and merged_tbl[1]:
                tables[sheet] = merged_tbl
                headers, rows = merged_tbl
                if ctrl and derived and len(derived[1]) > len(ctrl[1]):
                    warnings.append(
                        f"[Workbook] {sheet}: restored {len(derived[1]) - len(ctrl[1])} "
                        "commented/OFF row(s) from RPC sheet"
                    )
                elif not (ctrl and ctrl[1]) and derived and derived[1]:
                    warnings.append(
            ...
            elif sheet in wb.sheetnames and rpc_raw:
                warnings.append(f"[Workbook] Could not derive table from sheet RPC: {sheet}")

        pdsch_tbl = tables.get("PDSCH")
        if pdsch_tbl and pdsch_tbl[1]:
            pdsch_n = len(pdsch_tbl[1])
            flags: list[bool] = []
            raw_flags = simple.get("_detail_cc_on_flags")
            if isinstance(raw_flags, list):
                flags = [bool(x) for x in raw_flags]
            while len(flags) < pdsch_n:
                flags.append(True)
            flags = flags[:pdsch_n]
            if "PDSCH" in wb.sheetnames:
                pdsch_rpc = read_sheet_rpc_text(wb["PDSCH"]).strip()
                for idx in detect_commented_cc_row_indices_from_rpc(pdsch_rpc, "PDSCH"):
                    if 0 < idx <= len(flags):
                        flags[idx - 1] = False
            simple["_detail_cc_on_flags"] = flags
